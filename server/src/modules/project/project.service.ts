...
export async function listAll() {
  ...
  return projects.map((p) => ({
    ...toJSON(p),
    ownerName: (p.owner as any)?.name || 'Unknown',
  }));
}

export async function getByIdPublic(projectId: string) {
  const project = await Project.findById(projectId).populate('owner', 'name');
  if (!project) throw new NotFoundError('Project');
  return { ...toJSON(project), ownerName: (project.owner as any)?.name || 'Unknown' };
}
...
export async function create(name: string, description: string, userId: string) {
  const project = await Project.create({ name, description, owner: userId });
  return toJSON(project);
}

export async function update(projectId: string, userId: string, data: { name?: string; description?: string }) {
